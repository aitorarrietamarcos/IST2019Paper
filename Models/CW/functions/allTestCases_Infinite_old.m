function [totalInfinite] = allTestCases_Infinite(activationArray,TCData)
    ...
    for i = 1:length(TCData)
        for k = 1:length(TCData{1,i}.Infinite)
            infiniteAllTestCases(k) = infiniteAllTestCases(k) + TCData{1,i}.Infinite{1,k};
        end
        if(activationArray(i) == 1)
            for k = 1:length(TCData{1,i}.Infinite)
                newInfinite(k) = newInfinite(k) + TCData{1,i}.Infinite{1,k};
            end
        end
    end
