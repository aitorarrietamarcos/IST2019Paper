function [totalInstability] = allTestCases_Instability(activationArray,TCData)
    ...
    for i = 1:length(TCData)
        ...
        for k = 1:length(TCData{1,i}.Instability)
            instabilityAllTestCases(k) = instabilityAllTestCases(k) + TCData{1,i}.Instability(k);
        end
        if(activationArray(i) == 1)
            for k = 1:length(TCData{1,i}.Instability)
                newInstability(k) = newInstability(k) + TCData{1,i}.Instability(k);
            end
        end
    end
