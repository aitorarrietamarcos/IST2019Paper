function [totalDerivative] = allTestCases_Derivative_old(activationArray,TCData)
    ...
    for i = 1:length(TCData)
        for k = 1:length(TCData{1,i}.Derivative)
            derivativeAllTestCases(k) = derivativeAllTestCases(k) + TCData{1,i}.Derivative(k);
        end
        if(activationArray(i) == 1)
            for k = 1:length(TCData{1,i}.Derivative)
                newDerivative(k) = newDerivative(k) + TCData{1,i}.Derivative(k);
            end
        end
    end
